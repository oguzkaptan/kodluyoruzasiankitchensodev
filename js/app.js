...
function createButton(btnName) {
  ...
  return btn;
} // Bu fonksiyon; verilen parametreyi kullanarak buton oluşturur. Button id'si butonun içinde yazan parametreden gelir.
...
// Anasayfaya tüm menüyü ekleme;
// Burası menu dizisindeki obje bilgilerini createFood fonksiyonuna yollayarak menuleri oluşturur.
function allFoods() {
  ...
  menu.map((menu) => {
    ...
    allMenu += createFood(menu); //Tüm menü için koşul bildirmeksizin menü parametresiyle createFood fonksiyonunu çağırırız.
  });
...
// Korea yemekleri
function koreaFoods() {
  let allMenu = ""; // Değişkeni sıfırlamamızın nedeni all foods yemeklerinin sıfırlanıp Korea categorisinin eklenmesidir.
  menu.map((menu) => {
